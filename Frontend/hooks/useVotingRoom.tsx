...
export const useVotingRoom = (parameters: {
  ...
  const getContract = useCallback(() => {
    ...
    return new ethers.Contract(votingRoomAddress, contractABI.abi, ethersSigner);
  }, [ethersSigner, votingRoomAddress, contractABI]);

  const getReadOnlyContract = useCallback(() => {
    if (!ethersReadonlyProvider || !votingRoomAddress) return null;
    return new ethers.Contract(votingRoomAddress, contractABI.abi, ethersReadonlyProvider);
  ...
  const addCandidate = useCallback(async (
    ...
    try {
      const tx = await contract.addCandidate(roomCode, name, description, imageUrl);
      setMessage(`Waiting for transaction ${tx.hash}...`);
      const receipt = await tx.wait();
  ...
  const joinRoom = useCallback(async (roomCode: string, password: string = "") => {
    ...
    try {
      ...
      setMessage(`Error joining room: ${error instanceof Error ? error.message : String(error)}`);
      return false;
    } finally {
      setIsLoading(false);
    }
  ...
  // Cast Vote
  const castVote = useCallback(async (roomCode: string, candidateId: number) => {
    const contract = getContract();
    ...
    try {
      ...
      const input = instance.createEncryptedInput(votingRoomAddress, ethersSigner.address);
  ...
  // Get Room Info
  const getRoomInfo = useCallback(async (roomCode: string): Promise<Room | null> => {
    ...
    try {
      ...
      setMessage(`Error getting room info: ${error instanceof Error ? error.message : String(error)}`);
      return null;
    }
  }, [getReadOnlyContract]);

  // Get Candidates
  const getCandidates = useCallback(async (roomCode: string): Promise<Candidate[]> => {
    const contract = getReadOnlyContract();
    if (!contract) {
      setMessage("Contract not available");
      return [];
    ...
    try {
      ...
      for (let i = 0; i < candidateCount; i++) {
        const [name, description, imageUrl] = await contract.getCandidate(roomCode, i);
        candidates.push({
          id: i,
          name,
          description,
      ...
      setMessage(`Error getting candidates: ${error instanceof Error ? error.message : String(error)}`);
      return [];
    }
  }, [getReadOnlyContract]);

  // Check if user has voted
  const checkVotingStatus = useCallback(async (roomCode: string): Promise<{
    hasVoted: boolean;
    ...
    try {
      ...
      setMessage(`Error checking voting status: ${error instanceof Error ? error.message : String(error)}`);
      return { hasVoted: false, isParticipant: false };
    }
