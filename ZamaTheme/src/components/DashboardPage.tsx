...
import { ArrowLeft, Calendar, Users, Vote, Bell, Trophy, Clock, Eye, BarChart3, TrendingUp } from 'lucide-react';
...
export function DashboardPage({ onNavigate }: DashboardPageProps) {
  ...
  const [notifications, setNotifications] = useState<Notification[]>([
    {
      ...
      message: 'Team Building Location Vote results are now available. Beach Resort won with 60% votes.',
    ...
    },
    {
      ...
      message: 'Marketing Team Leader Election is almost full (12/15 participants).',
    ...
    },
    {
      ...
      message: 'Creative Ideas Contest voting ends in 24 hours. Make sure to cast your vote!',
    ...
    },
    {
      ...
      message: 'You have been invited to participate in Product Feature Priority voting.',
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      ...
      case 'room_full': return Users;
      case 'room_ending': return Clock;
      case 'new_room': return Vote;
      default: return Bell;
    }
  };
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-400">Manage your voting activities and track room performance</p>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-4 gap-6 mb-8">
            <Card className="bg-gray-800/50 border-gray-700/50">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-2xl text-blue-400 mb-1">{rooms.length}</div>
            ...
            <Card className="bg-gray-800/50 border-gray-700/50">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-2xl text-green-400 mb-1">{rooms.filter(r => r.myRole === 'creator').length}</div>
            ...
            <Card className="bg-gray-800/50 border-gray-700/50">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-2xl text-purple-400 mb-1">{voteHistory.length}</div>
            ...
            <Card className="bg-gray-800/50 border-gray-700/50">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-2xl text-orange-400 mb-1">{unreadCount}</div>
          ...
          <Tabs defaultValue="rooms" className="space-y-6">
            <TabsList className="bg-gray-800/50 border-gray-700/50">
              <TabsTrigger value="rooms" className="data-[state=active]:bg-white/10 data-[state=active]:text-white">
                My Rooms
              </TabsTrigger>
              ...
              <TabsTrigger value="notifications" className="data-[state=active]:bg-white/10 data-[state=active]:text-white">
                Notifications {unreadCount > 0 && <span className="ml-1 bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">{unreadCount}</span>}
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="rooms" className="space-y-6">
              ...
              <div className="grid lg:grid-cols-2 gap-6">
                {rooms.map((room) => (
                  <Card key={room.id} className="bg-gray-800/50 border-gray-700/50 hover:bg-gray-700/50 hover:border-gray-600/50 transition-all duration-300 hover:-translate-y-1">
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="outline" className="border-gray-600/50 text-gray-300">
                            {room.myRole === 'creator' ? 'Creator' : 'Participant'}
                          </Badge>
                          {room.hasPassword && (
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        <div className="flex items-center justify-between text-sm">
                          ...
                          {room.endTime && (
                            <div className="flex items-center gap-2 text-gray-400">
                              ...
                              <span>{room.status === 'completed' ? 'Ended' : 'Ends'} {room.endTime.toLocaleDateString()}</span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex gap-2">
                          <Button 
                            onClick={() => onNavigate('voting', { roomCode: room.code })}
                            variant="ghost"
                          ...
                          >
                            ...
                            {room.status === 'completed' ? 'View Results' : 'View Room'}
                          </Button>
                          {room.myRole === 'creator' && room.status === 'active' && (
                            <Button 
                              variant="outline"
            ...
            <TabsContent value="history" className="space-y-6">
              ...
              <div className="space-y-4">
                {voteHistory.map((vote) => (
                  <Card key={vote.id} className="bg-gray-800/50 border-gray-700/50">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            <h3 className="text-white">{vote.roomTitle}</h3>
                            <Badge className={vote.status === 'completed' ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30'}>
                              {vote.status === 'completed' ? 'Completed' : 'Pending'}
                            </Badge>
                          </div>
                          <div className="text-sm text-gray-400 mb-2">
                            Voted for: <span className="text-white">{vote.candidate}</span>
                          </div>
                          <div className="flex items-center gap-4 text-xs text-gray-500">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              {vote.votedAt.toLocaleDateString()} at {vote.votedAt.toLocaleTimeString()}
                            </div>
                          </div>
                        </div>
                        <Button
                          onClick={() => onNavigate('voting', { roomCode: vote.roomCode })}
                          variant="ghost"
                          className="text-gray-400 hover:text-white"
                        >
                          <Eye className="w-4 h-4" />
                      ...
                      <CardContent className="p-6">
                        <div className="flex gap-4">
                          <div className={`p-2 rounded-lg ${
                            notification.read ? 'bg-gray-700/50' : 'bg-blue-500/20'
                          }`}>
                          ...
                          <div className="flex-1">
                            <div className="flex items-start justify-between mb-2">
                              <h3 className={notification.read ? 'text-gray-300' : 'text-white'}>
                                {notification.title}
                              </h3>
                              <div className="flex items-center gap-2">
                            ...
                            {notification.roomCode && (
                              <Button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  onNavigate('voting', { roomCode: notification.roomCode });
                                }}
                                variant="ghost"
                                size="sm"
